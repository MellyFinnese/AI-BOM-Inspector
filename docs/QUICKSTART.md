...
- Keep graph guardrails on by default with `--enforce-graph-policy` (and `--env prod` in pipelines) so auto-discovered agents, tools, MCP configs, providers, and model IDs are checked for unsafe patterns.
