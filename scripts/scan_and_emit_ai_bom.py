...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the AI-BOM Inspector scan and emit a structured AI-BOM alongside the report."
    )
    ...
    command = [
        sys.executable, "-m", "aibom_inspector.cli_shim", "scan",
        "--requirements", args.requirements,
        "--models-file", args.models_file,
        "--format", args.format,
    ...
    build_rc = subprocess.run(
        [
            sys.executable,
            str(root / "scripts" / "build_ai_bom.py"),
            str(report_path),
            "--output", str(ai_bom_path),
        ],
        cwd=root,
    ).returncode
    if build_rc:
